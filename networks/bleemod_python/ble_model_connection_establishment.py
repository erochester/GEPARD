...
class BLEConnectionEstablishment:
    ...
    """
    Time model for BLE connection request procedures and for connection update procedures
    Implements a model for connection parameter update procedures and for connection request procedures
    """
    def ble_e_model_ce_get_duration_for_connection_procedure(self, establishment_or_update, scan_type, master_or_slave,
                                                           Tc_old, Tc_new):
        """
        Computes the time consumed by establishing a connection or updating the connection parameters of an
        existing connection.

        This function computes the tiem consumed by a connection request procedure or connection parameter update
        procedure.
        A connection request procedure occurs, after a scanner has received an ADV_IND advertising packet
        from an advertiser. After that, the scanner might respond with an CON_REQ packet, which is the first part of the
        connection establishment procedure.
        ...
        this value is ignored.
        :param Tc_new:	The future connection interval [s] after the connection request or establishment procedure
        :return: Time consumed for the parameter update
        """
        duration = 0
        duration_event = 0
        ble_scan = BLEScanner()
        ...
        if master_or_slave:
            ...
            if establishment_or_update:
                # Connection establishment procedure
                duration = ble_scan.ble_e_model_sc_get_duration_scan_event(0,
                                                                       BLEModelSCEventType.SC_EVENT_TYPE_CON_REQ_OFFSET,
                                                                       scan_type,
                                                                       BLE_E_MOD_CE_ADV_IND_PKG_LEN,
                                                                       BLE_E_MOD_CE_CON_REQ_LEN, 0, 0)
                duration += (0.00125 + ble_conn_est_params.BLE_E_MOD_CE_DTWO_CR(
                    Tc_new) + ble_conn_est_params.BLE_E_MOD_CE_DP(Tc_new))
            else:
                # Connection update procedure
                duration = ble_connected.ble_e_model_c_get_duration_event_same_payload(1, 0, 1,
                                                                                   BLE_E_MOD_CE_CON_UP_SLRSP_LEN,
                                                                                   BLE_E_MOD_CE_CON_UP_LEN,
                                                                                   BLE_E_MOD_CE_CON_UP_TXPOWER)
                duration_event = ble_connected.ble_e_model_c_get_duration_event_same_payload(1, 0, 1,
                                                                                             ...
                                                                                             BLE_E_MOD_CE_CON_UP_LEN,
                                                                                             BLE_E_MOD_CE_CON_UP_TXPOWER)
                duration += (Tc_old + 0.0 + ble_conn_est_params.BLE_E_MOD_CE_DP(
                    Tc_new) - duration_event)

        else:
            # Slave
            if establishment_or_update:
                # Connection establishment procedure
                duration = (0.00125 + ble_conn_est_params.BLE_E_MOD_CE_DTWO_CR(Tc_new) - (BLE_E_MOD_G_SCA * 2.0 / 1e6) *
                          (0.00125 + ble_conn_est_params.BLE_E_MOD_CE_DTWO_CR(Tc_new))) + \
                         (ble_conn_est_params.BLE_E_MOD_CE_DP(Tc_new) + (BLE_E_MOD_G_SCA * 2.0 / 1e6) *
                          (0.00125 + ble_conn_est_params.BLE_E_MOD_CE_DTWO_CR(Tc_new)))
            else:
                # Connection update procedure
                duration = ble_connected.ble_e_model_c_get_duration_event_same_payload(0, Tc_old, 1,
                                                                                   BLE_E_MOD_CE_CON_UP_LEN,
                                                                                   BLE_E_MOD_CE_CON_UP_SLRSP_LEN,
                                                                                   BLE_E_MOD_CE_CON_UP_TXPOWER)
                duration_event = ble_connected.ble_e_model_c_get_duration_event_same_payload(0, Tc_old, 1,
                                                                                             ...
                                                                                             BLE_E_MOD_CE_CON_UP_SLRSP_LEN,
                                                                                             BLE_E_MOD_CE_CON_UP_TXPOWER)
                duration += (Tc_old + 0.0 - (BLE_E_MOD_G_SCA * 2.0 / 1e6) * Tc_old - duration_event)
                duration += (ble_conn_est_params.BLE_E_MOD_CE_DP(Tc_new) + (
                            BLE_E_MOD_G_SCA * 2.0 / 1e6) * Tc_old)
